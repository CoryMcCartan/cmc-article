...
`cmc-article` includes a default `macros.tex` file with useful macros for math and statistics.
...
\begin{aligned}
   \vb{X} &\sim \Norm(\vbg{\mu}, \vbg\Sigma^2); \quad
    p(\vb x) = \frac{1}{\sqrt{(2\pi)^k\det(\vb*\Sigma)}}
        \exp\left(-\frac{1}{2} (\vb x-\vbg\mu)^\top \vbg\Sigma^{-1} (\vb x-\vbg\mu)\right) \\
    \E(Y) &= \sum_{y\in\mathcal{Y}} y\Pr(Y=y)
    = \sum_{y\in\mathcal{Y}} y\E(\mathbb{1}\{Y=y\})
\end{aligned}
$$ {#eq-first}

The package also includes an `assump` environment for typesetting assumptions which can be referenced by easy-to-remember abbreviations, as demonstrated in @thm-wlln below.
...
Then under @asm-iid and -@asm-fe, we have $\bar X_n\cvp\mu$ as $n\to\infty$.
:::
